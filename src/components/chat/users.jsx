      ...
      messageContext.allMessages.filter((elt) => {
        return (
          (elt.senderId == localStorage.getItem("userId") ||
            elt.senderId == receiverId) &&
          (elt.receiverId == localStorage.getItem("userId") ||
            elt.receiverId == receiverId)
        );
      })
    );
          ...
          if (!(localStorage.getItem("userId") == elt._id)) {
            return (
              <div
                key={index}
                className={
                  elt == messageContext.userReceiver
                    ? "flex  hover:bg-opacity-90 scale-105 cursor-pointer bg-person_background p-3 rounded-lg justify-start gap-3 active:bg-black"
                    : "flex hover:bg-black  hover:bg-opacity-10 hover:scale-105 cursor-pointer  p-3 rounded-lg justify-start gap-3"
                }
                onClick={() => {
